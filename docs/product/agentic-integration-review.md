...
| 2   | Context 是差異化，不是 UI             | 🔄 部分 | `content/context-packs/` + `api.context.*`、`specs/datacontracts/`、`catalog-search.dictionary`、`docs/architecture/ai-product/memory-policy.md` | request history / 機構記憶未累積；glossary 與 policy 未接入 agent context                  |
