...
## 0. 邊界備註（mirror repos）⚠️

`catalog-az-*` 系列（`calatlog-az-search`、`catalog-az-search-api`）與其他 `*-mirror` repo 為**公司產品的 mirror（唯讀）**：

| 規則             | 說明                                                                                                    |
| ---------------- | ------------------------------------------------------------------------------------------------------- |
| 業務隔離（絕對） | mirror 的實際業務內容、術語、資料、流程**絕不**直接成為本產品業務體系；本產品業務一律以 §1 自足定義表述 |
| 可參考範圍       | 僅限**業務解決情境**（problem–solution 模式）與**技術架構模式**                                         |
| 參考時保留判斷   | mirror 未必是最完整的技術架構，參考時需自行驗證取捨，不照搬                                             |
| 資安             | 不複製 mirror 的程式碼、設定、資料到本 repo                                                             |
...
| 優先 | 工作項            | 內容                                                                                    |
| ---- | ----------------- | --------------------------------------------------------------------------------------- |
| ①    | AI→手動預填       | LUI / dashboard 搜尋列生成篩選條件 → 寫入 URL（`?q=`+`?type=`）→ `/catalog-search` 接手 |
| ②    | AI 代填視覺標記   | items/申請表單 AI 代填欄位加標記，逐項確認/改寫                                         |
| ③    | `AiFallbackPanel` | 降級統一元件：逾時/攔截/信心不足 → 退手動畫面並保留輸入                                 |
