...
| 項目瀏覽          | LUI 引導到唯讀 items 列表                                        | `/items` 列表                                    | 手動 ✅ / AI 代填標記 ⬜                                                                                                                   |
...
| **S2 components** 🔄 | 2026-07-08：`@is_tess/components` 已建（9 元件上移為 canonical，portal 端暫為 vendored 副本）；**和色色盤**換裝（生成り紙底 + 藍主色 + 紙紋理，見 DESIGN_SYSTEM.md 色盤段）。待 npm 發布後 portal 改 import、清空 `app/components/ui`               | portal `app/components/ui` 清空、typecheck/E2E 綠                                         |
