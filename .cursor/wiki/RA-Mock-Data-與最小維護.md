...
- **目的**：可回溯、可重現、可控變更；對應「資料治理」中的版本精神（見 **Ref-資料治理與前端對等**）。
- **約定**：Mock 情境以 `app/test/datasets/v1/`、`v2/` 等版本管理；schema 或情境變更時新增版本並撰寫 `MIGRATION.md`，不直接覆寫舊 fixture；測試可指定 dataset version。
- **現況**：目前 fixture 仍寫在 handlers 內；目錄 `app/test/datasets/v1/` 已預留，可逐步將 fixture 遷出或由檔案載入。見 **Ref-Mock-Dataset-版本化**。
...
- 領域模型：**RA-領域模型-食物與食譜**
- Items API：**RA-Items-API**
- LUI：**RA-LUI-搜尋入口**
- 版號：**RA-Release-Notes與版號**
- 資料治理與 dataset 版本：**Ref-資料治理與前端對等**、**Ref-Mock-Dataset-版本化**
