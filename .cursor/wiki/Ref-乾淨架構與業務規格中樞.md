...
本專案朝向 **乾淨架構（Clean Architecture）**，並以 **業務規格（RA-\*）為專案中樞**：所有功能與資料流皆可追溯到規格，依賴方向由外而內，業務邏輯不依賴框架與 UI。

---

## 業務規格做專案中樞

- **中樞**：`.cursor/wiki/` 內 **RA-\*** 文件描述現況規格（業務流程、領域模型、API 行為、資料結構）。
- **原則**：新功能或 API 應先有對應的 RA- 或 ticket 描述；實作時 Contract、Mock、Test、UI 皆對齊該規格。
- **對照**：Ref-_ 為架構與技術參考；SOP-_ 為開發流程；RA-\* 為「做什麼」的單一來源。

| 規格類型   | 位置與前綴   | 用途                                   |
| ---------- | ------------ | -------------------------------------- |
| 業務／領域 | RA-\*        | 專案中樞；功能一覽、領域模型、API 規格 |
| 架構／流程 | Ref-_, SOP-_ | 怎麼實作、怎麼協作                     |
...
| 層級（由內而外）       | 本專案對應                                                               | 說明                                                           |
| ---------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------- |
| **Entities / 領域**    | RA-_ 文件、`app/shared/types/_`、契約中的 entity schema                  | 領域模型與共用型別；規格寫在 RA-\*，型別與 schema 為可執行表達 |
...
1. **新增功能**：先寫或更新 RA-\*（或 ticket），再走 Contract → Mock → Test → UI。
2. **跨模組**：共用邏輯放 `app/shared/*`，應用層整合放 `app/services/*`；領域知識以 RA-\* 與 shared types 為準。
...
- **RA-總覽**：功能一覽與專案定位
- **Ref-資料流與-Data-Driven-架構**：契約與請求流
- **SOP-資料測試導向開發-制度**：Spec→Contract→Mock→Test→UI 流程
- **collaboration-architecture.mdc**：目錄與資料流摘要
