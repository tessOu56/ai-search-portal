...
本文件說明：為何前端不追求「資料格式效率」（如 Delta/Parquet），而追求**可治理的資料層**；以及如何把「資料治理」精神落實在契約、mock 與版本化上，因應資料治理的變遷。
...
若要像 Delta 那樣「資料層可治理」，在本專案最接近的組合是：
...
- 見 **Ref-Mock-Dataset-版本化** 與 **RA-Mock-Data-與最小維護**。
...
- **SOP-資料測試導向開發-制度**：Spec → Contract → Mock → Test → UI、強制規則
- **Ref-Contract-與-Schema-規範**：Zod 契約與 template
- **Ref-API-與-Handler-對照**：API 與 handler 對應
- **Ref-Mock-Dataset-版本化**：dataset 目錄、版本、migration、測試指定版本
- **RA-Mock-Data-與最小維護**：種子、MSW、最小維護原則
