...
export type ListMetadataResult = {
  data: MetadataAssetSummaryContract[];
  pagination: {
    page: number;
    pageSize: number;
    total: number;
...
): MetadataAssetDetailContract | null {
  return loadAssetsForPack(packId).find((a) => a.id === assetId) ?? null;
}

export function resolveMetadataLineage(
  assetId: string,
  packId = DEFAULT_CONTEXT_PACK_ID
) {
  ...
  const all = loadAssetsForPack(packId);
  ...
  const edges = [
    ...
    ...asset.downstreamIds.map((target) => ({
      source: asset.id,
      target,
    })),
  ];
